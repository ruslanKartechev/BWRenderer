...
bool CreateFirstWindowAndInitGL(ProgramWindow* params);

void Win32WindowUpdate(const ProgramWindow& params);
...
{
    POINT win32MousePoint;

    if (GetCursorPos(&win32MousePoint)) {
        Input_SetAbsolutePosition(win32MousePoint.x, win32MousePoint.y);
        ScreenToClient(hwnd, &win32MousePoint);
        win32MousePoint.y = -(win32MousePoint.y - height);
        if (win32MousePoint.x < 0)
            win32MousePoint.x = 0;
        if (win32MousePoint.y < 0)
            win32MousePoint.y = 0;
        Input_SetMousePosition(win32MousePoint.x, win32MousePoint.y);
    }
...
LRESULT CALLBACK WinProcFunc(HWND hwnd,
                    ...
                    LPARAM lParam)
{
    ProgramWindow* window = nullptr;

    if (msg == WM_NCCREATE) {
        CREATESTRUCT* pCreate = (CREATESTRUCT*)lParam;
        window = static_cast<ProgramWindow*>(pCreate->lpCreateParams);
        SetWindowLongPtr(hwnd, GWLP_USERDATA, (LONG_PTR)window);
        return 1;
    } else {
        window = (ProgramWindow*)GetWindowLongPtr(hwnd, GWLP_USERDATA);
    }
...
bool ProgramWindow::CreateNativeWindowOpenGL(){
    ...
    UnregisterClass(dummyclassName, hInst);
    ...
    WNDCLASSEX wc = {
        ...
        hInst,
        nullptr, // icon
        LoadCursor(nullptr, IDC_ARROW), // cursor icon
        nullptr,
        nullptr,
        mainWinClass,
        nullptr
    ...
    HWND mainHwnd = CreateWindowEx(
            0, mainWinClass, name, windowStyle,
            posX, posY,
            width, height,
            nullptr,
            nullptr,
